...
__all__ = ['fMPO', 'merge_mpo_tensor_pair']
...
class fMPO:
    ...
    def __sub__(self, other):
        """
        Subtract another fMPO.
        """
        return add_mpo(self, other, alpha=-1)

    def __matmul__(self, other):
        """
        Multiply fMPO with another (composition along physical dimension).
        """
...
def add_mpo(op0: fMPO, op1: fMPO, alpha=1) -> fMPO:
    """
    ...
    if L == 1:
        ...
        for i in range(1, L):
            assert is_qsparse(op.A[i], [op.qd, -op.qd, op.qD[i], -op.qD[i+1]]), \
                'sparsity pattern of fMPO tensor does not match quantum numbers'
    return op


def multiply_mpo(op0: fMPO, op1: fMPO) -> fMPO:
    """
