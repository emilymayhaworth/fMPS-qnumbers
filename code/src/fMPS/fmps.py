...
__all__ = ['fMPS', 'merge_mps_tensor_pair', 'split_mps_tensor']


class fMPS:
    """
    Fermionic Matrix product state (fMPS) class.

    The i-th fMPS tensor has dimension `[d, D[i], D[i+1]]` with `d` the physical
...
def local_orthonormalize_right_qr(A: np.ndarray, Aprev: np.ndarray, qd: Sequence[int], qD: Sequence[Sequence[int]]):
    ...
    s = A.shape
    assert len(s) == 3
    q0 = qnumber_flatten([qd, qD[1]]) # q0 = qnumber_flatten([qd, -qD[1]]) CHANGED MINUS SIGN 
    Q, R, qbond = qr(A.reshape((s[0]*s[1], s[2])), q0, qD[0]) # AND HERE Q, R, qbond = qr(A.reshape((s[0]*s[1], s[2])), q0, -qD[0])
    ...
    return (A, Aprev, qbond) # AND HERE return (A, Aprev, -qbond)
...
def add_mps(mps0: fMPS, mps1: fMPS, alpha=1) -> fMPS:
    """
