...
class TestEvolution(unittest.TestCase):
    ...
    def test_tdvp_approximation(self):
        ...
        spin_tot = 2 #2 #TODO; HOW ALTER FOR FERMIH
        ...
        for i in range(L - 1):
            for q in qD[-1]:
                print("loop q", q + mpoH.qd)
            qD.append(np.sort(np.array([q + mpoH.qd for q in qD[-1]]).reshape(-1)))
        qD.append(np.array([1])) 
        print("qD HEREEEE", qD)

        # initial wavefunction as MPS with random entries
        psi = ptn.fMPS(mpoH.qd, qD, fill='random')
        ...
        self.assertEqual(psi.qD[-1][0], 1,
            msg='trailing bond quantum number must not change during orthonormalization')

            #self.assertEqual(psi.qD[-1][0], 2*spin_tot,
            #msg='trailing bond quantum number must not change during orthonormalization')
            #NOTE can orthonormalise if spin_tot = 1, 2, 0 and trailing bond qno is 1, 0,  but not if 2*spin_tot or 2 error in np.linalg.norm(A) !== 0
