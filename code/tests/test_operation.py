...
class TestOperation(unittest.TestCase):
    ...
    def test_vdot(self):
        ...
        psi = ptn.fMPS(np.zeros(d, dtype=int), [np.zeros(Di, dtype=int) for Di in [1, 3, 9, 13, 4, 1]], fill='random')
        chi = ptn.fMPS(np.zeros(d, dtype=int), [np.zeros(Di, dtype=int) for Di in [1, 4, 7, 8,  2, 1]], fill='random')
    ...
    def test_norm(self):
        ...
        psi = ptn.fMPS(np.zeros(d, dtype=int), [np.zeros(Di, dtype=int) for Di in [1, 3, 5, 8, 7, 2, 1]], fill='random')
    ...
    def test_operator_average(self):
        ...
        # create random matrix product state
        D = [1, 7, 26, 19, 25, 8, 1]
        psi = ptn.fMPS(qd, [np.random.randint(-1, 2, size=Di) for Di in D], fill='random')
        ...
        op = ptn.fMPO(qd, [np.zeros(Di, dtype=int) for Di in D], fill='random')
        ...
        # set bond quantum numbers to zero since otherwise,
        # sparsity pattern often leads to <psi | op | psi> = 0
        rho = ptn.fMPO(qd, [np.zeros(Di, dtype=int) for Di in D], fill='random')
